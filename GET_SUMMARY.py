...
class Get_Summary():
    ...
    def get_summary(self, file_list, path):
        ...
        summary_ws['b1'].value = 'V Freq[MHz]'
        ...
        summary_ws['h1'].value = 'FFT V freq'
        summary_ws['i1'].value = 'FFT V rms'
        summary_ws['j1'].value = 'FFT V DC abs'
        summary_ws['k1'].value = 'FFT I freq'
        summary_ws['l1'].value = 'FFT I rms'
        summary_ws['m1'].value = 'FFT I DC abs'
        ...
        for idx, excel_file in enumerate(excel_list):
            ...
            for i in range(9, 1, -1):
                if v_flag == 1 and i_flag == 1:
                    break
                elif ws.cell(13, i).value == 'V' and v_flag == 0:
                    sheet = 'FFT_' + ws.cell(21, i).value
                    ws_fft = wb[sheet]
                    try:
                        if ws_fft['f1'].value != 0:
                            summary_ws.cell(idx + 2, 8).value = ws_fft['f1'].value / 10**6
                    except:
                        summary_ws.cell(idx + 2, 8).value = 'None'
                    if ws_fft['f2'].value != 0:
                        summary_ws.cell(idx + 2, 9).value = ws_fft['f2'].value
                    try:
                        summary_ws.cell(idx + 2, 10).value = ws_fft['c2'].value / np.sqrt(2)
                    except:
                        summary_ws.cell(idx + 2, 10).value = ws_fft['c2'].value
                    v_flag = 1

                elif ws.cell(13, i).value == 'A' and i_flag == 0:
                    sheet = 'FFT_' + ws.cell(21, i).value
                    ws_fft = wb[sheet]
                    try:
                        if ws_fft['f1'].value != 0:
                            summary_ws.cell(idx + 2, 11).value = ws_fft['f1'].value / 10**6
                    except:
                        summary_ws.cell(idx + 2, 11).value = 'None'
                    if ws_fft['f2'].value != 0:
                        summary_ws.cell(idx + 2, 12).value = ws_fft['f2'].value
                    try:
                        summary_ws.cell(idx + 2, 13).value = ws_fft['c2'].value / np.sqrt(2)
                    except:
                        summary_ws.cell(idx + 2, 13).value = ws_fft['c2'].value
                    i_flag = 1
